...
First, set the App Groups for the main target by referring [here](https://docs.castled.io/developer-resources/sdk-integration/ios/push-notifications#3-adding-an-app-group-id).
*****Set the App Group ID for the extension.*****
 
  Enable App Groups for your notification service extension target and use the same App Group ID that was selected for the app target (as mentioned in the previous steps).
