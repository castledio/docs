...
**1. Adding an App Group ID**

App Groups facilitate the sharing of data between multiple apps developed by the same organization and their extensions. This is not required for push notifications per se but is relevant if you have a **Notification Content Extension** for rich notifications or need to manage a shared container of data.
...
<Note>
It is advised to enabling Remote Notifications for efficient **Uninstall tracking** and **Delivery Status tracking** even incases app is idle or in the background.
</Note>

**3. Enabling Push Notifications Capability**
...
![](/images/ios-app-target.png)

### Registering Push Notification 

In iOS 10 and above, to handle push notifications, you need to implement the following method from **UIApplicationDelegate** protocol and the below mentioned three steps for registering push notification.
