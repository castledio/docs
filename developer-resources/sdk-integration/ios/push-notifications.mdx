...
<Note>
To ensure proper integration, we recommend setting up a separate App Group ID specifically for Castled. The format should be group.{app bundle id}.Castled. Make sure to enable this same App Group ID for all targets where Castled is utilized.
</Note>
