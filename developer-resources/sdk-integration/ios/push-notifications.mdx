...
<Note>
It is advised to enable Remote Notifications for efficient **Uninstall tracking** and better **Push impressions**, even when the app is in the background.
</Note>
...
<Note>
To ensure proper integration, we recommend setting up a separate App Group ID specifically for Castled.The format should be **group.your_bundle_id.castled**. Make sure to enable this same App Group ID for all targets where Castled is utilizd.
</Note>
...
class AppDelegate: UIResponder, UIApplicationDelegate , UNUserNotificationCenterDelegate {
    ...
    func registerForPush() {
        Castled.sharedInstance.requestPushPermission(showSettingsAlert: false)
    }
}
```
...
- (void)registerForPush {
    [[Castled sharedInstance] requestPushPermissionWithShowSettingsAlert:FALSE];
}
```
</CodeGroup>
...
We recommend configuring the **appGroupId** and using the [Notification Service Extension](/developer-resources/sdk-integration/ios/rich-push-notification) for enhanced tracking of notification impressions, accurate badge counts, and media support.
...
If you prefer to have more granular control over notification handling or need to comply with specific architectural requirements, disable swizzling by referring to the section below.
...
**type** : A CastledNotificationType indicating the type of notification. The values include 'push' or 'inapp'.
...
<Note>
To report dismiss events for push notifications, include the **customDismissAction** in the **UNNotificationCategory** options. Make sure to define your notification categories with this option, as shown in the code snippet above.
</Note>
