...
<AccordionGroup>
  ...
  <Accordion title="Common error codes when sending test push notification">
   ...
   2. Send to sandbox selection in **Send Test** push:
      When sending test pushes, make sure that the **Send to Sandbox** option is appropriately selected. If you're testing in a sandbox environment, enable this option to direct notifications to the sandbox APNS servers. 
