...
<Accordion title="Push notification is not showing in the device after sending from Castled dashboard">
  ...
  this issue. 1. Ensure mobile device has network connection. 2. Make sure
  `enablePush` config is set to `true`. 3. Firebase configs are setup in Castled
  dashboard. 4. Make sure app has granted push notification permission. You can
  navigate to **Settings > Notifications > App Notifications > Your App** to see
  the notification permission status. 5. If you are running the app in debug
  mode, make sure app is running. It can be in focus or out of focus. 6. If
  sending notification using a **Campaign** or **Workflow**, check out the
  **Reports** section for the **Campaign** or **Workflow**. If `Skipped` counter
  ...
  support. 7. Verify that the app is not force stopped. For e.g. some devices
  ...
  `stopped=false` to receive push. 8. If you already tried the above checklist
  and the problem still persists, reach out to `tech@castled.io` or ping us in
  our Slack channel.
</Accordion>
