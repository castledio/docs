...
<Accordion title="Push notification is not showing in the device after sending from Castled dashboard">
   There can be a lot of reasons why this can happen after a successfull SDK initialization. You can use the following checklist as a starter for debugging this issue.
   
   1. Make sure `enablePush` config is set to `true`.
   2. Firebase configs are setup in Castled dashboard. 
   3. Make sure app has granted push notification permission. You can navigate to **Settings > Notifications > Your App** to see the notification permission status.
   4. If you are running the app in debug mode, make sure app is running. It can be in focus or out of focus.
   5. Verify that the app is not force stopped. For e.g. some devices have energy saving mode enabled that could kill the app. 
